...
// TODO: get rid of (or keep?) chalker stuff
...
func submitTx(destinationURL, alias, domain string, satoshis uint64, p2pTx *paymail.P2PTransaction) (response *paymail.P2PTransactionResponse, err error) {

	// Start the request
	displayHeader(chalker.DEFAULT, fmt.Sprintf("Submitting tx to p2p endpoint for %s...", chalk.Cyan.Color(alias+"@"+domain)))
	...
	// Create the address resolution request
	if response, err = client.SendP2PTransaction(
		destinationURL,
		alias,
		domain,
		p2pTx,
	...
	// Success
	chalker.Log(chalker.SUCCESS, fmt.Sprintf("Submitted txid: %s with note: %s", response.TxID, response.Note))
